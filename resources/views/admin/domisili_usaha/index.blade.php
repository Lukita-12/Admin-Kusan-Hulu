<x-admin-layout>
    ...
    <x-table.container variant="main">
        <div class="bg-blue-400/80 w-full flex flex-wrap justify-between items-center px-4 py-2 rounded-t-lg gap-2">

            <!-- Form Search -->
            <form method="GET" action="{{ route('admin.domisili_usaha.search') }}" class="flex items-center gap-2">
            ...
            <form method="GET" action="{{ route('admin.domisili_usaha.print') }}" target="_blank" class="flex items-center gap-2">
            ...
            <form method="GET" action="{{ route('admin.domisili_usaha.filter') }}" id="filterForm"
                ...
                <label for="filter_status" class="font-medium text-slate-100">Filter:</label>
                <select name="status" id="status" class="outline-none text-slate-700 text-base rounded-sm"
                                        ...
                                        @if ($domisiliUsahas->currentPage() === 1 && $loop->first)
                                            @can('acceptOrReject', $domisiliUsaha)
                                                @if ($domisiliUsaha->status === 'Diajukan')
                                                    <x-table.form
                                                        action="{{ route('admin.domisili_usaha.accept', $domisiliUsaha) }}">
                                                        @method('PATCH')
                                                        <x-table.button variant="accept" type="submit">Terima</x-table.button>
                                                    </x-table.form>

                                                    <x-table.form
                                                        action="{{ route('admin.domisili_usaha.reject', $domisiliUsaha) }}">
                                                        @method('PATCH')
                                        ...
                                        @if ($domisiliUsahas->currentPage() === 1 && $loop->first)
                                            @can('completeOrEditOrDelete', $domisiliUsaha)
                                                @if ($domisiliUsaha->status !== 'Selesai')
                                                    <x-table.form
                                                        action="{{ route('admin.domisili_usaha.complete', $domisiliUsaha) }}">
                                                        @method('PATCH')
                                                        <x-table.button variant="complete" type="submit">Selesai</x-table.button>
                                                    </x-table.form>

                                                    <x-table.form
                                                        action="{{ route('admin.domisili_usaha.destroy', $domisiliUsaha) }}">
                                                        @method('DELETE')
